...
if any(criterio == '' for criterio in criterios):
    ...
    for i in range(num_criterios):
        for j in range(i + 1, num_criterios):
            valor_comparacao = st.number_input(f"Para você, o quanto o critério {criterios[i]} é mais importante que  {criterios[j]} (escala 1-9)", min_value=0.25, max_value=9.0, step=0.1, format="%.1f")
